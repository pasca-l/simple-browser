...
impl JsParser {
    ...
    fn parameter_list(&mut self) -> Vec<Option<Rc<Node>>> {
        ...
        // checks if the parameter list is given with a starting '('
        match self.t.next() {
            Some(t) => match t {
    ...
    // Statement ::= ReturnStatement | VariableStatement | ExpressionStatement
    ...
    // BinaryExpression ::= LeftHandSideExpression ( AdditiveOperator AssignmentExpression )*
    // AdditiveOperator ::= <"+"> | <"-">
