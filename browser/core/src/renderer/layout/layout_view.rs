...
use crate::renderer::html::helper::api::get_target_element_node;
...
impl LayoutView {
    pub fn new(root: Rc<RefCell<Node>>, cssom: &StyleSheet) -> Self {
        // layout tree is replaced under the body tag
    ...
    // calculates position and size for each node
    ...
    fn calculate_node_size(node: &Option<Rc<RefCell<LayoutObject>>>, parent_size: LayoutSize) {
        if let Some(n) = node {
            // for block elements, width is determined first
...
fn build_layout_tree(
    ...
    if let Some(n) = target_node {
        ...
        if first_child.is_none() && original_first_child.is_some() {
            ...
            // loops until no reachable sibling node exists
            loop {
        ...
        let obj = match layout_object {
            Some(ref obj) => obj,
            None => panic!("render object should exist"),
        };
